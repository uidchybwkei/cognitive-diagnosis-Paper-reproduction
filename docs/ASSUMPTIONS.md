...
* 论文符号中在不同位置同时使用了 **q\_{ik}** 和 **q\_{ki}** 两种记法。
...
* 论文指出诊断目标 **c\_j** 和 **α\_{jk}** 位于 **[0,1]** 区间内，但模型采用高斯似然与高斯先验，且未说明在优化过程中是否进行投影或裁剪操作。
* 本实现默认对 **c** 和 **α****不施加取值约束**（不进行裁剪）。
* 同时提供了一个可配置选项，可在训练过程中将 **c** 和 **α** 裁剪至 **[0,1]**，默认关闭，用于稳定性实验。
...
* **论文写为 **α^{new} = α^{old} - r\_2 \* (∂/∂α) g(α)**，并定义 **g(α) = ∂F/∂α**。**
* 本实现将 Step-2 解释为对目标函数 **F** 进行标准的梯度下降：
  * α^{new} = α^{old} - r\_2 \* g(α)**，其中 **g(α) = ∂F/∂α**。**
